package com.finwin.doorstep.digicob.home.jlg.split_transaction.other_details
...
import com.finwin.doorstep.digicob.databinding.OtherDetailsFragmentBinding
...
import com.finwin.doorstep.digicob.home.jlg.split_transaction.other_details.adapter.ChargesAdapter
                ...
                JlgAction.SPLIT_TRANSACTION_SUCCESS->
                {
                    SweetAlertDialog(context, SweetAlertDialog.SUCCESS_TYPE)
                        ...
                        .setConfirmClickListener {
                            ...
                            (activity as SplitTransactionActivity?)?.viewModel?.mAction?.value= JlgAction(JlgAction.JLG_CLEAR_DATA)
                        }
                        .show()
                }
            }
        ...
        viewModel.mAction.observe(viewLifecycleOwner, Observer {
            when(it.action)
            {
                ...
                JlgAction.CLICK_SUBMIT_FROM_JLG_SPLIT_OTHER_DETAILS->
                {
                    ...
                    (activity as SplitTransactionActivity?)?.viewModel?.instrumentType?.set(viewModel.instrumenTypeSelected.get())
                    (activity as SplitTransactionActivity?)?.viewModel?.instrumentType?.set(viewModel.instrumenTypeSelected.get())
                    (activity as SplitTransactionActivity?)?.viewModel?.instrumentNumber?.set(viewModel.instrumentNumber.get())
                    (activity as SplitTransactionActivity?)?.viewModel?.instrumentDate?.set(viewModel.submitInstrumentDate.get())
                    (activity as SplitTransactionActivity?)?.viewModel?.totalAmount?.set(viewModel.totalAmount.get())

                    //(activity as SplitTransactionActivity?)?.viewModel?.initLoading(context)

                    (activity as SplitTransactionActivity?)?.viewModel?.callSpitTransactionApi(context)
                }
